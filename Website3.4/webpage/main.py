...
def create_request():
    if request.method == 'POST':
        ...
        if image.filename != '':
            ...
            image_path = os.path.join('webpage/static/pictures/', filename)  # Save to the "pictures" folder
        ...
        new_req = requests(title=title, descr=descr, quantity=quantity, image=image_path, user_id=current_user.id)
